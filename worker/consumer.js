...
try {
  ...
  console.log('   Place serviceAccountKey.json in project root');
}
...
async function saveToFirebase(processedTick) {
  ...
  try {
    await db.collection('processed_ticks').add(processedTick);
...
async function run() {
  ...
  await consumer.run({
    eachMessage: async ({ message }) => {
      ...
      console.log(`🔧 [${totalProcessed}] ${anomalyMarker}${tick.symbol}: $${tick.price} | MA: $${movingAvg} | Vol: ${volatility}%`);
